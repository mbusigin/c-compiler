...
static void declare_builtins(void) {
    ...
    // stderr, stdout, stdin - FILE* pointers
    // On macOS, stderr is actually __stderrp, stdout is __stdoutp, stdin is __stdinp
    Type *void_ptr_type = type_create(TYPE_POINTER);
    void_ptr_type->base = type_create(TYPE_VOID);
    
    // Add __stderrp, __stdoutp, __stdinp (actual symbols on macOS)
    ...
    symtab_add(current_symtab, "__stdinp", SYMBOL_VARIABLE, void_ptr_type);
    
    // Also add stderr/stdout/stdin as aliases (the parser will use __stderrp/__stdoutp/__stdinp)
