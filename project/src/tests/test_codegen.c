// Integration tests for the compiler - tests the full compilation pipeline
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/wait.h>

static int tests_run = 0;
static int tests_passed = 0;

#define ASSERT(cond, msg) do { \
    tests_run++; \
    if (cond) { \
        tests_passed++; \
        printf("  [PASS] %s\n", msg); \
    } else { \
        printf("  [FAIL] %s\n", msg); \
    } \
} while(0)

// Test compilation of simple programs
int test_simple_return() {
    printf("\nTest: Simple return 42\n");
    
    const char *src = "int main() { return 42; }";
    FILE *f = fopen("/tmp/cc_test_simple.c", "w");
    fprintf(f, "%s", src);
    fclose(f);
    
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "./compiler /tmp/cc_test_simple.c -o /tmp/cc_test_simple 2>&1");
    
    int result = system(cmd);
    if (result != 0) {
        printf("  [SKIP] Compilation failed\n");
        return 0;
    }
    
    // Run the compiled program
    result = system("/tmp/cc_test_simple");
    int exit_code = WEXITSTATUS(result);
    
    ASSERT(exit_code == 42, "Program should return 42");
    return 1;
}

int test_arithmetic() {
    printf("\nTest: Arithmetic expression (1 + 2) * 3 = 9\n");
    
    const char *src = "int main() { return (1 + 2) * 3; }";
    FILE *f = fopen("/tmp/cc_test_arith.c", "w");
    fprintf(f, "%s", src);
    fclose(f);
    
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "./compiler /tmp/cc_test_arith.c -o /tmp/cc_test_arith 2>&1");
    
    int result = system(cmd);
    if (result != 0) {
        printf("  [SKIP] Compilation failed\n");
        return 0;
    }
    
    result = system("/tmp/cc_test_arith");
    int exit_code = WEXITSTATUS(result);
    
    ASSERT(exit_code == 9, "Program should return 9");
    return 1;
}

int test_function_call() {
    printf("\nTest: Function call add(3, 4) = 7\n");
    
    const char *src = "int add(int a, int b) { return a + b; } int main() { return add(3, 4); }";
    FILE *f = fopen("/tmp/cc_test_func.c", "w");
    fprintf(f, "%s", src);
    fclose(f);
    
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "./compiler /tmp/cc_test_func.c -o /tmp/cc_test_func 2>&1");
    
    int result = system(cmd);
    if (result != 0) {
        printf("  [SKIP] Compilation failed\n");
        return 0;
    }
    
    result = system("/tmp/cc_test_func");
    int exit_code = WEXITSTATUS(result);
    
    ASSERT(exit_code == 7, "Program should return 7");
    return 1;
}

int test_nested_calls() {
    printf("\nTest: Nested function calls\n");
    
    const char *src = "int double(int x) { return x * 2; } int main() { return double(double(3)); }";
    FILE *f = fopen("/tmp/cc_test_nested.c", "w");
    fprintf(f, "%s", src);
    fclose(f);
    
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "./compiler /tmp/cc_test_nested.c -o /tmp/cc_test_nested 2>&1");
    
    int result = system(cmd);
    if (result != 0) {
        printf("  [SKIP] Compilation failed\n");
        return 0;
    }
    
    result = system("/tmp/cc_test_nested");
    int exit_code = WEXITSTATUS(result);
    
    ASSERT(exit_code == 12, "Program should return 12 (double(double(3)))");
    return 1;
}

int test_comparison() {
    printf("\nTest: Comparison operations\n");
    
    const char *src = "int main() { return (5 > 3) ? 1 : 0; }";
    FILE *f = fopen("/tmp/cc_test_cmp.c", "w");
    fprintf(f, "%s", src);
    fclose(f);
    
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "./compiler /tmp/cc_test_cmp.c -o /tmp/cc_test_cmp 2>&1");
    
    int result = system(cmd);
    if (result != 0) {
        printf("  [SKIP] Compilation failed\n");
        return 0;
    }
    
    result = system("/tmp/cc_test_cmp");
    int exit_code = WEXITSTATUS(result);
    
    ASSERT(exit_code == 1, "Program should return 1 (5 > 3)");
    return 1;
}

int test_printf() {
    printf("\nTest: printf Hello World\n");
    
    const char *src = "#include <stdio.h>\nint main() { printf(\"Hello, World!\\n\"); return 0; }";
    FILE *f = fopen("/tmp/cc_test_printf.c", "w");
    fprintf(f, "%s", src);
    fclose(f);
    
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "./compiler /tmp/cc_test_printf.c -o /tmp/cc_test_printf 2>&1");
    
    int result = system(cmd);
    if (result != 0) {
        printf("  [SKIP] Compilation failed\n");
        return 0;
    }
    
    result = system("/tmp/cc_test_printf");
    int exit_code = WEXITSTATUS(result);
    
    ASSERT(exit_code == 0, "Program should exit with 0");
    return 1;
}

int main(int argc, char **argv) {
    (void)argc;
    (void)argv;
    printf("===========================================\n");
    printf("Compiler Integration Tests\n");
    printf("===========================================\n");
    
    test_simple_return();
    test_arithmetic();
    test_function_call();
    test_nested_calls();
    test_comparison();
    test_printf();
    
    printf("\n===========================================\n");
    printf("Integration Tests Summary:\n");
    printf("  Tests run: %d\n", tests_run);
    printf("  Tests passed: %d\n", tests_passed);
    printf("  Tests failed: %d\n", tests_run - tests_passed);
    printf("===========================================\n");
    
    return tests_passed == tests_run ? 0 : 1;
}
